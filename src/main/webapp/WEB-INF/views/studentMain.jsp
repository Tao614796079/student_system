...
<div style="padding-left: 2%;padding-right: 2%;">
    <button type="button" class="btn btn-default pull-right"
            onclick="window.location.href='${pageContext.request.contextPath}/goAdd'">添加学生信息
    </button>
    <br><br>
    <table class="table-bordered text-center" width="100%">
        ...
        <c:forEach items="${studentList}" var="bean">
            <tr>
                ...
                <td><a href="${pageContext.request.contextPath}/deleteStudent?id=${bean.id}&cur_page=${cur_page}">删除</a>/<a
                        href="${pageContext.request.contextPath}/goUpdate?id=${bean.id}&cur_page=${cur_page}">修改</a>
                </td>
            </tr>
        </c:forEach>
    </table>
...
<script>
    function next_page() {
        ...
        if (cur < count) {
            window.location.href = "${pageContext.request.contextPath}/studentMain?cur_page=" + (cur + 1);
        } else {
            alert("已经是最后一页了！");
        }
    }
